...
teste_ods = data.frame(ods_teste$id,as.integer(as.character(teste$.pred_class)),as.integer(ods_teste$ods_16))
teste_ods = teste_ods %>%
  purrr::set_names(c("id","pred","real"))

write.csv2(teste_ods,"dados/teste_ods.csv", row.names = T)

#verificação manual da acurácia
