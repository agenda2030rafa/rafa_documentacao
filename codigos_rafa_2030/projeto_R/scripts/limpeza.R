...
dados_hotsite <- read_excel("dados/hotsite.xlsx")
...
#leitura das peças - pasta acórdãos para teste
caminho = "dados/peças/acord"
...
#usar as primeiras 500 palavras, no máximo (opcional)
for(i in 1:nrow(df)){
  df$texto[i] = word(df$texto[i],
                           1, 
                           num_words(df$texto[i],500))
}

#verificando o número de palavras por texto (< 500 agora)
tokenizers::count_words(df$texto)
...
#criação - base de treino - ods 16 apenas
